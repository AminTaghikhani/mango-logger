...
class Logger {
  ...
  errorToFile(title, error, req = null) {
    ...
    Utils.checkDirSync(`${this.root}/${title}s`);
    Utils.createFileSync(`${this.root}/${title}s/${title}${EXT}`, LOGSTRINGBUILDER('error', error, req));
  }
  ...
  objToFile(title, type, obj, req = null) {
    ...
    Utils.checkDirSync(`${this.root}/${title}s`);
    Utils.createFileSync(`${this.root}/${title}s/${title}${EXT}`, LOGSTRINGBUILDER(type, JSON.stringify(obj), req));
  }
}
